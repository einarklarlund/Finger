...
public class UIManager : MonoBehaviour
{
    ...
    public void OnInteractableSpeechBegan(InteractableSpeechBeganSignal signal)
    {
        ...
        StartCoroutine(BeginSpeech(signal.interactableSpeech));
    }

    IEnumerator BeginSpeech(InteractableSpeech interactableSpeech)
    {
        while(interactableSpeech.HasNextDialogue())
        {
            _skipDialogue = false;

            Text dialogueText = interactableSpeech.isPlayerSpeaking ? 
                _playerDialogueText : _interactableDialogueText;

            // set dialogue menus active/inactive
            _playerDialogueMenu.SetActive(interactableSpeech.isPlayerSpeaking);
            _interactableDialogueMenu.SetActive(!interactableSpeech.isPlayerSpeaking);
            interactableSpeech.SetTalkingAnimationActive(!interactableSpeech.isPlayerSpeaking);

            // animate the text and wait for animation to complete
            yield return AnimateText(dialogueText, interactableSpeech.GetNextDialogue());

            interactableSpeech.SetTalkingAnimationActive(false);
        ...
        _signalBus.Fire(new InteractableSpeechEndedSignal()); 

        yield break;
    }
    
    IEnumerator AnimateText(Text text, string dialogue, bool isSkippable = true)
    { 
        for (int dialogueIndex = 0; !_skipDialogue && dialogueIndex < dialogue.Length; dialogueIndex++)
        {
        ...
        // wait for a moment because idk it causes problems with detecting clicks if you dont
        yield return new WaitForSeconds(1 / _textSpeed);
