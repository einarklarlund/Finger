// There was a divide by 0 warning from using the normalize() function in GetDistPlane()
// 		added a vector to the parameter of normalize()
...
{
	// "modifier" will be a very small vector if rot is 0 vector. 
	// otherwise, modifier will be 0 
	float m = (-abs(sign(length(rot))) + 1) * 0.0000001;
	float3 modifier = float3(m, m, m);
