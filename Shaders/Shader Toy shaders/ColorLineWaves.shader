...
// The "COLORS" array was initialized with parenthesis rather than curly brackets.
// 		Changed parenthesis to curly brackets.
// 
// In segmentColor(), "COLORS.length()" was being used (a function that doesn't exist).
// 		The length of of the "COLORS" array is always 6 because that's its size at instantiation.
// 		Replaced "COLORS.length()" with 6.
// 
// In frag(), the vector "uv" was being multiplied with a matrix using the * operator.
// 		Replaced the expression with mul(uv, matrix)
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed3 segmentColor(fixed2 st) {
				return COLORS[int(random(st) * fixed(6))];
			}
